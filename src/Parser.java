...
HashMap<String, FunctionNode> programMap = new HashMap<String, FunctionNode>();	
	public Node parse() throws NoEndOfLineException, MultipleAdditionException, IncorrectIndentationException, IncorrectSyntaxException{
		...
		System.out.println("finished parsing, the remaining token should be the placeholder indent --> (" + objectList.get(0).getToken()+")" + objectList.get(0).getStringValue());
		return program;
	}
	...
	public IfNode parseIf() throws MultipleAdditionException, IncorrectIndentationException, IncorrectSyntaxException {//parses out the if function
		IfNode.ifTypes ifType = setIfType(objectList.get(0));//to see what type of if we have
		...
		IfNode ifParsed = new IfNode(condition, ifStatements, ifType);
		if(peek(0)!=Token.tokenType.IF&& peek(0)!=Token.tokenType.ELSIF && peek(0)!=Token.tokenType.ELSE) return ifParsed;
		IfNode chainedIf = parseIf();
		ifParsed.setNext(chainedIf);
		return ifParsed;
	}
	
	public IfNode.ifTypes setIfType(Token ifType){
		if(ifType.getToken()==Token.tokenType.IF) {
			return IfNode.ifTypes.IF;
		}
		if(ifType.getToken()==Token.tokenType.ELSE) {
			return IfNode.ifTypes.ELSE;
		}
		if(ifType.getToken()==Token.tokenType.ELSIF) {
			return IfNode.ifTypes.ELSEIF;
		}
		return null;
	}
	...
	public VariableNode processVariables() throws NoEndOfLineException {//need to handle consts and variable declarations below parameters
		...
		if(matchAndRemove(Token.tokenType.CONSTANTS)!=null) {
			...
			if(matchAndRemove(Token.tokenType.CHARLITERAL)!=null) {
				CharacterNode charValue = new CharacterNode(value.getStringValue().charAt(0));
				type = "char";
	...
	public Node factor() throws MultipleAdditionException{//usually looking for a number, but will find left parens that will yield an expression of its own
		...
		if(matchAndRemove(Token.tokenType.NUMBER)==null && matchAndRemove(Token.tokenType.MINUS)==null&& matchAndRemove(Token.tokenType.LPAREN)==null
		&&matchAndRemove(Token.tokenType.IDENTIFIER)==null&& matchAndRemove(Token.tokenType.STRINGLITERAL)==null&&matchAndRemove(Token.tokenType.CHARLITERAL)==null&&matchAndRemove(Token.tokenType.TRUE)==null&&matchAndRemove(Token.tokenType.FALSE)==null)
		{ 
			return null;
		}
		...
		if(numberToken.getToken()==Token.tokenType.STRINGLITERAL) {
			StringNode string = new StringNode(numberToken.getStringValue());
			return string;
		}
		if(numberToken.getToken()==Token.tokenType.CHARLITERAL) {
			CharacterNode character = new CharacterNode(numberToken.getStringValue().charAt(0));
			return character;
		}
		if(numberToken.getToken()==Token.tokenType.TRUE) {
			BooleanNode bool = new BooleanNode(true);
			return bool;
		}
		if(numberToken.getToken()==Token.tokenType.FALSE) {
			BooleanNode bool = new BooleanNode(false);
			return bool;
		}
